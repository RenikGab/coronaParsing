...
idents = ["Министерство здравоохранения Кировской области информирует", "По данным министерства здравоохранения Кировской области", 
  "Количество проводимых тестов на коронавирус", "За прошедшие сутки лабораториями региона выполнено", "Для обследования пациентов с коронавирусными пневмониями", 
  "По данным мониторинга за минувшие сутки", "В Кировской области увеличиваются темпы"]

exports.DownloadAllPages = async function(end_page) {
  proxy.ProxyInit()
  try {
    ...
    for (let ii = 1; ii <= end_page; ii+=20)
    {
      ...
      if (ii > 1)
        URL += URL_from + "/" + ii;
      console.log("DownloadAllPages " + URL)
      // needle.get(URL, function (err, res) {
      const res = await needle("get", URL)
        //if (err) throw err;
        if (res.statusCode !== 200) {
          throw new Error('failed needle not 200');
        }
        ...
        news_entries.each(async function() {
          links = $(this).get(0)
            ...
            if (link.attribs)
              if ("onclick" in link.attribs) {
                ...
                if (str.length > 1) {
                  ...
                  const res = await needle("get", URL_begin + str[1])
                    //if (err) throw err;
                    if (res.statusCode !== 200) {
                      throw new Error('failed needle not 200');
                    }
                    ...
                    for (let ident of idents) {
                      ...
                      if (begin_paragraph.indexOf(ident, 0) >= 0) {
                        ...
                        console.log("DownloadAllPages " + fileName)
    ...
    //throw new Error('failed needle');
    console.log("neele error " + error);
   }
}
